...
function procesarTiposInteres(tipoCalculo, denominador) {
    ...
    if (diasExactos < 0 || diasAprox < 0) { res.innerHTML = `<div class="alert alert-danger mt-3 fw-bold">La fecha final debe ser posterior.</div>`; return; }
...
// ==========================================
function actualizarDecimal(id) { const valor = parseFloat(document.getElementById(`val-${id}`).value); const unidadTasa = document.getElementById(`unidad-tasa-${id}`); const resDecimal = document.getElementById(`res-decimal-${id}`); if (!isNaN(valor)) { let decimal = valor / 100; if (unidadTasa) { let m = parseFloat(unidadTasa.value) || 1; decimal = decimal / m; } resDecimal.value = decimal.toFixed(6); } else { resDecimal.value = ""; } }
function alternarBaseDias(id) { const u = document.getElementById(`unidad-${id}`).value; const c = document.getElementById(`base-dias-container-${id}`); const i = document.getElementById(`val-${id}-meses`); if (c) { if (u === "dias") { c.classList.remove("d-none"); } else { c.classList.add("d-none"); } } if (i) { if (u === "anios_meses") { i.classList.remove("d-none"); } else { i.classList.add("d-none"); i.value = ""; } } }
function actualizarTiempo(id) { const iA = document.getElementById(`val-${id}`); const iM = document.getElementById(`val-${id}-meses`); const u = document.getElementById(`unidad-${id}`).value; const cD = document.getElementById(`res-tiempo-${id}`); let v1 = parseFloat(iA.value) || 0; let v2 = iM && !iM.classList.contains('d-none') ? parseFloat(iM.value) || 0 : 0; if (u === "anios_meses") { cD.value = (v1 + (v2 / 12)).toFixed(4); } else if (u === "semestres") { cD.value = (v1 / 2).toFixed(4); } else if (u === "cuatrimestres") { cD.value = (v1 / 3).toFixed(4); } else if (u === "trimestres") { cD.value = (v1 / 4).toFixed(4); } else if (u === "bimestres") { cD.value = (v1 / 6).toFixed(4); } else if (u === "meses") { cD.value = (v1 / 12).toFixed(4); } else if (u === "dias") { const base = document.getElementById(`base-${id}`) ? parseFloat(document.getElementById(`base-${id}`).value) || 360 : 360; cD.value = (v1 / base).toFixed(4); } }
...
function actualizarTiempoFechas(id) { const dI = parseInt(document.getElementById(`dia-ini-${id}`).value); const mI = document.getElementById(`mes-ini-${id}`).value; const aI = parseInt(document.getElementById(`anio-ini-${id}`).value) || 0; const dF = parseInt(document.getElementById(`dia-fin-${id}`).value); const mF = document.getElementById(`mes-fin-${id}`).value; const aF = parseInt(document.getElementById(`anio-fin-${id}`).value) || 0; const cD = document.getElementById(`res-tiempo-${id}`); if (!isNaN(dI) && !isNaN(dF) && aI > 0 && aF > 0) { try { let vI = tablaTiempoExacto.año1[mI][dI - 1]; let vF = tablaTiempoExacto.año1[mF][dF - 1]; if (vI === undefined || vF === undefined) return; let anioDif = aF - aI; let diasTotales = 0; if (anioDif === 0) { diasTotales = vF - vI; } else if (anioDif === 1) { vF = tablaTiempoExacto.año2[mF][dF - 1]; diasTotales = vF - vI; } else { let diasAniosEnteros = (anioDif - 1) * 365; vF = tablaTiempoExacto.año2[mF][dF - 1]; diasTotales = diasAniosEnteros + (vF - vI); } const esBisiesto = (y) => (y % 4 === 0 && (y % 100 !== 0 || y % 400 === 0)); let numM1 = Object.keys(tablaTiempoExacto.año1).indexOf(mI); let numM2 = Object.keys(tablaTiempoExacto.año1).indexOf(mF); for (let y = aI; y <= aF; y++) { if (esBisiesto(y)) { let iRango = (y === aI) ? new Date(aI, numM1, dI) : new Date(y, 0, 1); let fRango = (y === aF) ? new Date(aF, numM2, dF) : new Date(y, 11, 31); let feb29 = new Date(y, 1, 29); if (feb29 >= iRango && feb29 <= fRango) { diasTotales++; } } } cD.value = (diasTotales / 360).toFixed(4); } catch(e) {} } }
...
function procesarCalculoSimple(key) {
    ...
    let resultadoFinal = 0; let etiqueta = ""; let esPorcentaje = false; let htmlVisual = "";

    switch(key) {
        case 'I': resultadoFinal = P * i * t; etiqueta = "Interés (I)"; htmlVisual = `I = ${P} × ${i.toFixed(6)} × ${t.toFixed(4)}`; break;
        case 'P': resultadoFinal = I / (i * t); etiqueta = "Capital (P)"; htmlVisual = `P = ${I} / (${i.toFixed(6)} × ${t.toFixed(4)})`; break;
        case 'i': resultadoFinal = (I / (P * t)) * 100; etiqueta = "Tasa (i)"; esPorcentaje = true; htmlVisual = `i = [ ${I} / (${P} × ${t.toFixed(4)}) ] × 100`; break;
        case 't': resultadoFinal = I / (P * i); etiqueta = "Tiempo (t)"; htmlVisual = `t = ${I} / (${P} × ${i.toFixed(6)})`; break;
        case 'S': resultadoFinal = P * (1 + (i * t)); etiqueta = "Monto (S)"; htmlVisual = `S = ${P} × [ 1 + (${i.toFixed(6)} × ${t.toFixed(4)}) ]`; break;
        case 'Ps': resultadoFinal = S / (1 + (i * t)); etiqueta = "Capital (P)"; htmlVisual = `P = ${S} / [ 1 + (${i.toFixed(6)} × ${t.toFixed(4)}) ]`; break;
        case 'is': resultadoFinal = ((S / P) - 1) / t * 100; etiqueta = "Tasa (i)"; esPorcentaje = true; htmlVisual = `i = [ (${S} / ${P} - 1) / ${t.toFixed(4)} ] × 100`; break;
        case 'ts': resultadoFinal = ((S / P) - 1) / i; etiqueta = "Tiempo (t)"; htmlVisual = `t = (${S} / ${P} - 1) / ${i.toFixed(6)}`; break;
        case 'D': resultadoFinal = S * d * t; etiqueta = "Descuento (D)"; htmlVisual = `D = ${S} × ${d.toFixed(6)} × ${t.toFixed(4)}`; break;
        case 'Pd': resultadoFinal = S - D; etiqueta = "Valor Actual (P)"; htmlVisual = `P = ${S} - ${D}`; break;
        case 'P_dt': resultadoFinal = S * (1 - (d * t)); etiqueta = "Valor Actual (P)"; htmlVisual = `P = ${S} × [ 1 - (${d.toFixed(6)} × ${t.toFixed(4)}) ]`; break;
        case 'S_dt': resultadoFinal = P / (1 - (d * t)); etiqueta = "Valor Nominal (S)"; htmlVisual = `S = ${P} / [ 1 - (${d.toFixed(6)} × ${t.toFixed(4)}) ]`; break;
        case 'd_despeje': resultadoFinal = ((1 - (P / S)) / t) * 100; etiqueta = "Tasa Desct. (d)"; esPorcentaje = true; htmlVisual = `d = [ (1 - ${P}/${S}) / ${t.toFixed(4)} ] × 100`; break;
        case 't_despeje': resultadoFinal = (1 - (P / S)) / d; etiqueta = "Tiempo (t)"; htmlVisual = `t = (1 - ${P}/${S}) / ${d.toFixed(6)}`; break;
        case 'd_equiv': resultadoFinal = (i / (1 + (i * t))) * 100; etiqueta = "Tasa Eq. (d)"; esPorcentaje = true; htmlVisual = `d = [ ${i.toFixed(6)} / (1 + ${i.toFixed(6)} × ${t.toFixed(4)}) ] × 100`; break;
        case 'i_equiv': resultadoFinal = (d / (1 - (d * t))) * 100; etiqueta = "Tasa Eq. (i)"; esPorcentaje = true; htmlVisual = `i = [ ${d.toFixed(6)} / (1 - ${d.toFixed(6)} × ${t.toFixed(4)}) ] × 100`; break;
    }
...
function procesarCalculoCompuesto(key) {
    ...
    let n = 0; let t_anios = 0;
    const nStr = document.getElementById('val-n')?.value || "";
    if (nStr.includes("enteros")) {
        let n_ent = parseFloat(nStr.split(' ')[0]) || 0; let m_rest = parseFloat(document.getElementById('comp-frac-meses').value) || 0; let m_base = parseFloat(document.getElementById('comp-frac-base').value) || 1;
        n = n_ent + (m_rest / m_base);
    } else { n = parseFloat(nStr.split(' ')[0]) || parseFloat(document.getElementById('val-n')?.value) || 0; }
    ...
    switch(key) {
        case 'S_comp': resultadoFinal = P * Math.pow((1 + i), n); etiqueta = "Monto Compuesto (S)"; htmlVisual = `S = ${P} × (1 + ${i.toFixed(6)})<sup>${n.toFixed(4)}</sup>`; break;
        case 'P_comp': resultadoFinal = S * Math.pow((1 + i), -n); etiqueta = "Valor Actual (P)"; htmlVisual = `P = ${S} × (1 + ${i.toFixed(6)})<sup>-${n.toFixed(4)}</sup>`; break;
        case 'n_comp': resultadoFinal = Math.log(S / P) / Math.log(1 + i); etiqueta = "Número de Periodos (n)"; htmlVisual = `n = ln(${S} / ${P}) / ln(1 + ${i.toFixed(6)})`; break;
        case 'i_comp': resultadoFinal = (Math.pow((S / P), (1 / n)) - 1) * 100; etiqueta = "Tasa Efectiva por Periodo (i)"; esPorcentaje = true; htmlVisual = `i = [ (${S} / ${P})<sup>1 / ${n.toFixed(4)}</sup> - 1 ] × 100`; break;
        case 'j_m': resultadoFinal = m * (Math.pow((1 + (j_dec / m_prima)), (m_prima / m)) - 1) * 100; etiqueta = "Tasa Nominal Eq. (j)"; esPorcentaje = true; htmlVisual = `j = ${m} × [ (1 + ${j_dec.toFixed(6)} / ${m_prima})<sup>${m_prima} / ${m}</sup> - 1 ] × 100`; break;
        case 'i_nominal': resultadoFinal = ((Math.pow((1 + (j_dec / m)), (t_anios * m)) - 1) / t_anios) * 100; etiqueta = "Tasa Efectiva Anual (i)"; esPorcentaje = true; htmlVisual = `i = [ (1 + ${j_dec}/${m})<sup>${t_anios.toFixed(4)} × ${m}</sup> - 1 ] / ${t_anios.toFixed(4)}`; break;
        case 'j_desde_i': resultadoFinal = m * (Math.pow((1 + (i_manual * t_anios)), (1 / (t_anios * m))) - 1) * 100; etiqueta = "Nominal desde Efectiva (j)"; esPorcentaje = true; htmlVisual = `j = ${m} × [ (1 + ${i_manual} × ${t_anios.toFixed(4)})<sup>1 / (${t_anios.toFixed(4)} × ${m})</sup> - 1 ]`; break;
        ...
        case 'P_frac':
            ...
            if (modoFrac === 'exacto') {
                ...
                htmlVisual = key === 'S_frac' ? `S = ${P} × (1 + ${i.toFixed(6)})<sup>${n.toFixed(4)}</sup>` : `P = ${S} × (1 + ${i.toFixed(6)})<sup>-${n.toFixed(4)}</sup>`;
            } else {
                let n_ent = Math.floor(n); let m_rest = parseFloat(document.getElementById('comp-frac-meses').value) || 0; let m_base = parseFloat(document.getElementById('comp-frac-base').value) || 1;
                ...
                htmlVisual = key === 'S_frac' ? `S = [ ${P} × (1 + ${i.toFixed(6)})<sup>${n_ent}</sup> ] × [ 1 + (${i.toFixed(6)} × (${m_rest}/${m_base})) ]` : `P = [ ${S} × (1 + ${i.toFixed(6)})<sup>-${n_ent}</sup> ] × [ 1 + (${i.toFixed(6)} × (${m_rest}/${m_base})) ]`;
            }
